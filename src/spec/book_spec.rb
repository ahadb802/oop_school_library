...
describe Book do
  ...
  describe "private methods" do
    ...
    describe "#save_id_counter" do
      before do
        File.delete('id_counter.json') if File.exist?('id_counter.json')
      end

      it "saves the ID counter to a file" do
        book.send(:next_id)
        expect(File).to receive(:write).with('id_counter.json', { counter: book.instance_variable_get(:@id_counter)}.to_json)
